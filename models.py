...
# --- Example Usage (Check for None and Empty Strings) ---
if __name__ == "__main__":
    NUM_PROFILES_TO_GENERATE = 5
    RANDOM_SEED = 2025

    generated_data = generate_customer_data(NUM_PROFILES_TO_GENERATE, seed=RANDOM_SEED)

    if generated_data:
        print("\n--- Checking First Profile for None Values or Empty Strings ---")
        first_profile = generated_data[0]
        found_issue = False
        issues_list = []

        def check_for_issues(data, path=""):
            global found_issue, issues_list
            is_issue_here = False
            if isinstance(data, dict):
                for k, v in data.items():
                    new_path = f"{path}.{k}" if path else k
                    if v is None:
                        issues_list.append(f"None at: {new_path}")
                        is_issue_here = True
                    elif isinstance(v, str) and v == "":
                        issues_list.append(f"Empty String at: {new_path}")
                        is_issue_here = True
                    # Check list/dict only if no issue found at current level
                    elif isinstance(v, (dict, list)):
                        check_for_issues(v, new_path)

            elif isinstance(data, list):
                 # Check list emptiness if needed (e.g. items list could be empty if num_items=0)
                 # if not data: issues_list.append(f"Empty List at: {path}")
                 for i, item in enumerate(data):
                    new_path = f"{path}[{i}]"
                    check_for_issues(item, new_path)
            # Update global flag if an issue was found at this level or deeper
            if is_issue_here:
                found_issue = True

        check_for_issues(first_profile)
        if not found_issue:
            print("  PASSED: No None values or empty strings found in the first profile.")
        else:
             print("  FAILED: Issues found (None or empty strings):")
             for issue in issues_list:
                 print(f"    - {issue}")


        print("\n--- Example Purchase (First Profile) Showing Placeholders ---")
        if first_profile.get("recent_purchases"):
            example_purchase = next((p for p in first_profile["recent_purchases"] if p.get("tracking_number") == PLACEHOLDER_TRACKING), first_profile["recent_purchases"][0])
            print(json.dumps(example_purchase, indent=2))
        else:
            print("  No purchases to show example.")

        print("\n--- Example Customer Showing Circle Number ---")
        example_cust = first_profile["customer"]
        print(f"  Is Member: {example_cust.get('target_circle_member')}")
        print(f"  Circle Number: '{example_cust.get('target_circle_number')}'")

    # output_filename = f"customer_data_nonempty_{NUM_PROFILES_TO_GENERATE}.json"
    # try:
    #     with open(output_filename, 'w') as f: json.dump(generated_data, f, indent=2)
    #     print(f"\n--- Successfully saved generated data to {output_filename} ---")
